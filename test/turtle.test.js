define(['jquery', 'sinon', 'src/turtle', 'src/utils'], function($, sinon, Turtle, utils) {
   QUnit.module('Turtle graphics', {
    ...
    QUnit.test('turtle has optional drawing options parameter', function(assert) {
        var turtle = new Turtle(this.$canvas, { lineLength: 10 });
        assert.equal(turtle.lineLength, 10);
    });
    ...
    QUnit.test('turtle can rotate', function(assert) {
        var turtle = new Turtle(this.$canvas);
        turtle.right();
        assert.equal(turtle.angle, utils.degToRad(90));
        turtle.left();
        assert.equal(turtle.angle, utils.degToRad(0));
    });
    ...
    QUnit.test('reseting resets angle', function(assert) {
        var turtle = new Turtle(this.$canvas);
        turtle.angle = utils.degToRad(90);
        turtle.reset();
